...
class ExamConfigTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.q_table.setStyleSheet("""
            ...
            QHeaderView::section { background:#21262d; color:#8b949e; padding:10px; border:none; font-weight:bold; font-size:12px; }
    ...
    def _load_questions(self):
        ...
        for q in questions:
            ...
            q_text = q["question"]
            q_item = QTableWidgetItem(q_text[:80] + ("…" if len(q_text) > 80 else ""))
            ...
            edit_btn.setStyleSheet(BTN_STYLE.format(bg="#1f6feb", hover="#388bfd"))
            edit_btn.clicked.connect(lambda _, row_q=q: self._open_edit_dialog(row_q))
            t.setCellWidget(r, 6, edit_btn)

            # Delete button
            del_btn = QPushButton("Del")
            del_btn.setStyleSheet(BTN_STYLE.format(bg="#da3633", hover="#f85149"))
            del_btn.clicked.connect(lambda _, qid=q["id"]: self._delete_question(qid))
            t.setCellWidget(r, 7, del_btn)
    ...
    def _open_pdf_importer(self):
        """Open the PDF / file MCQ importer dialog."""
    ...
    def _on_questions_imported(self, count: int):
        """Called when the importer has saved questions to the DB."""
        self._load_questions()
        self.config_changed.emit()
